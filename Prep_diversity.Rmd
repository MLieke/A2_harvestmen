...
#q=1
bacteria <- read.csv("Data/Plot_hill_min_16S.csv") %>% rename(D1 = qD)
protists <- read.csv("Data/Plot_hill_1_18S.csv") %>% rename(D1 = qD)
nematodes <- read.csv("Data/Plot_hill_1_18S_N.csv") %>% rename(D1 = qD)
fungi <- read.csv("Data/Plot_hill_min_ITS.csv") %>% rename(D1 = qD)
...
# check per study site, spread in 1D
# ggplot(data = nematodes, aes(x = factor(unique_site), y = qD, colour = factor(unique_site))) + geom_point()

#q=0
bacteria_q0 <- read.csv("Data/Plot_hill_0_16S.csv") %>% rename(D0 = qD)
protists_q0 <- read.csv("Data/Plot_hill_0_18S.csv") %>% rename(D0 = qD)
nematodes_q0 <- read.csv("Data/Plot_hill_0_18S_N.csv") %>% rename(D0 = qD)
fungi_q0 <- read.csv("Data/Plot_hill_0_ITS.csv") %>% rename(D0 = qD)
...
# #q=2
# bacteria_q2 <- read.csv("Data/Plot_hill_2_16S.csv") %>% rename(D2 = qD)
# protists_q2 <- read.csv("Data/Plot_hill_2_18S.csv") %>% rename(D2 = qD)
# nematodes_q2 <- read.csv("Data/Plot_hill_2_18S_N.csv") %>% rename(D2 = qD)
# fungi_q2 <- read.csv("Data/Plot_hill_2_ITS.csv") %>% rename(D2 = qD)
# springtails_q2 <- read.csv("Data/Hill_2_COI_co.csv") %>% rename(D2 = qD) %>% rename(dna_code = Assemblage)
# mites_q2 <- read.csv("Data/Hill_2_COI_mi.csv") %>% rename(D2 = qD) %>% rename(dna_code = Assemblage)

## combine 0D and 1D into one dataframe - 
## would require that D1 and D0 are again called qD actually
# bacteria <- rbind(bacteria, bacteria_q0)
# fungi <- rbind(fungi, fungi_q0)
# protists <- rbind(protists, protists_q0)
# nematodes <- rbind(nematodes, nematodes_q0)
# springtails <- rbind(springtails, springtails_q0)
# mites <- rbind(mites, mites_q0)
```
...
# maybe the below shouldn't be included here, if we anyway only look at averaged abundances across sessions
# for(i in 1:length(dflist)) {
#     specablist[[i]] <- dflist[[i]] %>%
#       select(c(code, species, abundance)) %>%
#       pivot_wider(names_from = species,
#                 values_from = abundance) %>%
#       column_to_rownames(var = "code")
#     names(specablist)[i] <- paste0(names(dflist[i]), "_specab")
# }

### potentially, we may want to sum the two traps per study site, because they may not always be labelled properly 1 and 2, see above where we would do that
...
# diversitylist_D2 <- diversitylist
```
...
saveRDS(diversitylist_D0, "diversitylist_D0.rds")
```
...
saveRDS(diversitylist_D1, "diversitylist_D1.rds")
```

## 0D, 1D in table

```{r richness, shannon table}
#D0
output_q0 <- data.frame()
for(m in names(diversitylist_D0)){
  df <- diversitylist_D0[[m]]
  sum <- df %>% group_by(landuse) %>% summarise(across(D0, list(min=min, Q1=~quantile(., probs = 0.25),
                    median=median, mean=mean, Q3=~quantile(., probs = 0.75),
                    max=max, sd=sd),  .names = "{.fn}"))
  sum <- mutate(sum, organism = m)
  output_q0 <- bind_rows(output_q0,round_df(sum, digits=1))
}

w <- output_q0 %>% select(organism, landuse, mean, sd)

#D1
output_q1 <- data.frame()
for(m in names(diversitylist_D1)){
  df <- diversitylist_D1[[m]]
  sum <- df %>% group_by(landuse) %>% summarise(across(D1, list(min=min, Q1=~quantile(., probs = 0.25),
                    median=median, mean=mean, Q3=~quantile(., probs = 0.75),
                    max=max, sd=sd),  .names = "{.fn}"))
  sum <- mutate(sum, organism = m)
  output_q1 <- bind_rows(output_q1,round_df(sum, digits=1))
}

x <- output_q1 %>% select(organism, landuse, mean, sd)
```
