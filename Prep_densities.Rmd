...
# count data: abundance and activity-density
...
mites <- read.csv("Data/Mites_P2.csv") %>%
  as.data.frame() %>%
  # transform to abundance per g of dry soil instead of kg
  mutate(abundance = abundance/1000)

springtails <- read.csv("Data/Springtails_P2.csv") %>%
  as.data.frame() %>%
  # g instead of kg
  mutate(abundance = abundance/1000)

nematodes <- read.csv("Data/Nematodes_P2.csv") %>%
  as.data.frame() %>%
  # g instead of kg
  mutate(abundance = abundance/1000)
...
saveRDS(dflist, "densitylist.rds")
# compare with densitylist made in model diagnostics to see if up to date: this is OK
```
